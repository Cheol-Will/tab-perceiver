...
def load_result():
    ...
    model_types = ["FTPerceiver", "LightGBM"]
    ...
    for model_type in model_types:
        for scale_type in scale_types:
            ...
            for idx in index_list:
                path = f"output1/{task_type}/{scale_type}/{idx}/{model_type}.pt"
...
def plot_result(result_list, task_type):
    ...
    # columns = LightGBM, FTTransformer, so on.
    ...
    plt.text(0.52, 0.97, "LightGBM Better", fontsize=12, fontweight='bold', color='blue')
    plt.text(0.72, 0.53, "Deep model Better", fontsize=12, fontweight='bold', color='red')

    plt.xlabel("ROC-AUC for deep tabular models")
    plt.ylabel("ROC-AUC for LightGBM")
    plt.title(f"Deep Models vs LightGBM across {task_type} Task")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
